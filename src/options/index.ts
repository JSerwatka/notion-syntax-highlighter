...
function capitalizeFirstLetters(string: string) {
  ...
  const arrayOfWordsCapitalized = arrayOfWords.map(
...
const adjustThemeName = (theme: string) => {
  const themeNameNormalized = theme.replace('base16/', '').replace('-', ' ');

  return capitalizeFirstLetters(themeNameNormalized);
};

const themeList = themes.map(
  (theme) =>
    `<div class="theme-checkbox"><input type="checkbox" id="${theme}" name=${theme}/><label for="${theme}">${adjustThemeName(
      theme
    )}</label></div>`
);
